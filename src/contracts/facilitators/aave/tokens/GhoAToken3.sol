...
import {GhoVariableDebtToken3} from './GhoVariableDebtToken3.sol';

/**
 * @title Aave GHO ERC20 AToken
 * @author Aave
 * @notice Implementation of the interest bearing token for the Aave protocol modified for GHO
 */
contract GhoAToken3 is
  ...
  IGhoAToken
{
  ...
  // NEW Gho STORAGE
  GhoVariableDebtToken3 internal _ghoVariableDebtToken;
  ...
  function initialize(
    ...
    (ghoVariableDebtTokenAddress, _ghoTreasury) = abi.decode(params, (address, address));
    _ghoVariableDebtToken = GhoVariableDebtToken3(ghoVariableDebtTokenAddress);
  ...
  /// @inheritdoc IGhoAToken
  function setGhoTreasury(address newTreasury) external override onlyPoolAdmin {
    address previousTreasury = _ghoTreasury;
    _ghoTreasury = newTreasury;
    emit GhoTreasuryUpdated(previousTreasury, newTreasury);
  }
